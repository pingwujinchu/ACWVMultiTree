...
public class CCFP implements Serializable{
	...
	public CCFP(Instances insts, Instances onlyclass, double minsup, double minconv, int necSupport,
		...
		for(int i = 0 ; i < onlyClass.numInstances() ; i++){
			classNumber[(int)onlyClass.instance(i).value(0)] ++;
		}
	}
...
//					}
	...
	public int calculateTotal(HeaderNode hn,FastVector []headertable){
		int totalNum = 0;
		for(int i = 0 ; i < numClass; i++){
			if(headertable[i].contains(hn)){
				totalNum += ((HeaderNode)headertable[i].elementAt(headertable[i].indexOf(hn))).count;
			}
		}
		return totalNum;
	}
	...
	private double compConv(int classCount,int total, int j) {
		double conf = (double)classCount/total;
		if(conf==1)
			conf = 0.999;
	...
	private void ccfpGrow(FastVector prefix, Tree t, FastVector headertable,
		...
		if(t.hasOnePath()==true){
			...
			for(int i=0;i<size;i++){
				...
				int total = calculateTotal(hn, this.headertable);
				for(int j=0;j<numClass;j++){
					if(hn.classcount[j]>necSupport){
						flag  =false;
						conv = compConv(hn.classcount[0],total,j);
						if(conv>=minconv)
							for(int k=0;k<=i;k++){
								len = prefix.size()+k+1;
